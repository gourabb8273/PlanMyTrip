 ...
 * API_TOKEN_KEY - apiToken is a static key where one can call from postman by passing it in request header
...
const requiredAuthToken = (req, res, next) => {
  try {    
    ...
    logger.error(
      `Error while processing authentication middleware ${JSON.stringify(
        error
      )}`
    );
    next({
      message: `Error while processing authentication middleware, Please provide a valid token `,
      error,
    });
  }
};
