...
app.get("/", (req, res) => {
  res.status(200).json({ message: "Payment service is called" });
});
...
app.listen(port, () => {
  logger.info(`Payment service is listening at http://localhost:${port}`);
});
...
app.on("listening", () => {
  ...
  logger.info("Payment service is registered successfuly");
});
