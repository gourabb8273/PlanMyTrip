...
/**
 * MiddleWare for verifying a user role (non admin)
...
const requiredUser = (req, res, next) => {
  try {
    ...
    const role = verifypayload && verifypayload.role === userRoles.USER;
    if (!role)
      return res
        .status(400)
    ...
    logger.error(`Error while processing user middleware ${JSON.stringify(error)}`);
    return res.status(400).json({ message: "Please authenticate with a valid token", error });
  }
};
